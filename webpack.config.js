...
function addSpritePlugin(configItem) {
    const existingPlugins = Array.isArray(configItem.plugins) ? configItem.plugins : []
    return {...configItem, plugins: [...existingPlugins, new SpriteLoaderPlugin()]}
}

module.exports = config.map((configItem) => {
    if (configItem.name === configNames.CLIENT || configItem.name === configNames.SERVER) {
        return addSpritePlugin({
            ...configItem,
    ...
    return addSpritePlugin(configItem)
})
